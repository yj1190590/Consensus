...
## Notes
<sup>1</sup>	VRF, "Verifiable Random Function", is a random function using private key to encrypt and public key to verify, and its application can avoid that the generated votes are predicted by attackers in advance. [←](#21-Votes-Generation) <br>

<sup>2</sup>	In order to apply the function, we should save the temporary hesitation period into block headers for the other chains to obtain from their own views. [←](#244-Voting-retracement-and-delayed-finalization) <br>
