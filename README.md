...
(1) A random number operation is conducted based on constants (such as timestamp and block hash) at miner nodes. When the expected results reach a certain target and meet the requirements to generate blocks, it is denoted as $Block_Proof < target * votes * efficiency$, where target refers to difficulty, which controls the speed of block generation; votes refers to the votes owned by miners and is directly proportional to the possibility of block generation; and efficiency refers to the efficiency of block generation, which is affected by various factors and which will be later introduced in detail.<br>
